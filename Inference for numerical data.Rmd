...
The births is the correct answer
</ol>
...
The correct answer is: Range of birth weights of babies born to non-smoker mothers is greater than that of babies born to smoker mothers.
</ol>
...
The correct answer is: $H_0: \mu_{smoking} = \mu_{non-smoking}$; $H_A: \mu_{smoking} \ne \mu_{non-smoking}$
</ol>
...
</div> We have a p-value smaller than alpha (significance level), therefore we reject the null hypothesis 
...
The correct answer is: We are 95% confident that babies born to nonsmoker mothers are on average 0.05 to 0.58 pounds heavier at birth than babies born to smoker mothers.
</ol>
```{r habit-weight-ci}
...
The correc answer is: (38.0952 , 38.5742)
</ol>
```{r weeks-ci-99}
# type your code for Question 6 here, and Knit
inference(y=weeks, x=NULL, data=nc, statistic = "mean", type="ci", conf_level=.99, method ="theoretical")
```
...
inference(y=weeks, x= NULL, data= nc, statistic = "mean", type= "ci", conf_level = .90,
          method = "theoretical")
```
...
The next method works since we can appreciate that the max of the young mothers is in 34, and the min of the older mothers is 35. Therefore the cutoff point is 35.
```{r cutoff-mature}
...
<li> $H_0: \mu_{premie} = \mu_{non-premie}$; $H_A: \mu_{premie} \ne \mu_{non-premie}$ </li>
Our H0 is simply that there is no difference between a premie weight and a non-premie weight.
Our Ha is simply that there is a difference between a premie weight and a non-premie weight.
```{r pick-your-own}
# type your code for the Exercise here, and Knit
inference(y=weight, x= premie, data= nc, type= "ht", statistic = "mean", null = 0, alternative = "twosided", method = "theoretical")

inference(y=weight, x= premie, data = nc, type= "ci", statistic = "mean", method = "theoretical")
```

Our test gave us a p-value=.0001 at a significance level of alpha=.05. Therefore there is enough evidence for rejecting H0. Which implies that premies tend to have lower weight than non-premies. In fact we are 95% confident that no-premies weight between (2.007 , 2.655) pound more than premies.
