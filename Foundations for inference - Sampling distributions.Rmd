...
ggplot(data = ames, aes(x = area)) +
  geom_histogram(binwidth = 250, fill = "light blue", color= "black")
```
...
This one(<li> 50\% of houses in Ames are smaller than 1,499.69 square feet. </li>)
...
samp1%>%
  summarise(mean(area),sd(area), median(area), IQR(area), quantile(area,.25), quantile(area, .75), min(area), max(area))
...
This one(<li> Sample size of 1000. </li>)
</ol>
...
ggplot(data = sample_means50, aes(x = x_bar)) +
  geom_histogram(binwidth = 20, fill= "light blue", color="black")
```
...
sample_means50%>%
  summarise(mean(x_bar), sd(x_bar), median(x_bar), mode(x_bar),
            IQR(x_bar),quantile(x_bar, .25), quantile(x_bar, .75))

ggplot(data = sample_means50, aes(x= x_bar)) +
  geom_histogram(binwidth = 3, fill= "light green", color= "black")
...
This one(<li> Each element represents a mean square footage from a simple random sample of 10 houses. </li>)
...
ggplot(data= sample_means15, aes(x= x_bar)) +
  geom_histogram(binwidth = 10000, fill= "light blue", color= "black")
...
ggplot(data = sample_means150, aes(x = x_bar)) +
  geom_histogram(binwidth= 1500, fill= "light green", color= "black")
```
6. Which of the following is false? 
<ol>

This one (<li> The variability of the sampling distribution with the smaller sample size (`sample_means50`) is smaller than the variability of the sampling distribution with the larger sample size (`sample_means150`). </li>
